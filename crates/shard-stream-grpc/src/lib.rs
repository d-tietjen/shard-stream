...
use shard_stream_protocol::{
    ...
    FetchMode as EngineFetchMode, FetchRequest as EngineFetchRequest, ProducerIdentity,
};
    ...
    async fn append(
        ...
        tokio::spawn(async move {
            loop {
                ...
                let request = match decode_append(message) {
                    Ok(request) => request,
                    Err(status) => {
...
fn decode_fetch(request: v1::FetchRequest) -> Result<EngineFetchRequest, Status> {
    Ok(EngineFetchRequest {
        ...
        topic_id: TopicId::new(decode_u128(request.topic_id, "topic_id")?),
        partition_id: LogicalPartitionId::new(request.partition_id),
        start_offset: LogicalOffset::new(decode_u128(request.start_offset, "start_offset")?),
...
fn encode_append(response: shard_stream_protocol::AppendResponse) -> v1::AppendResponse {
    v1::AppendResponse {
        ...
        first_offset: Some(encode_u128(response.first_offset.get())),
        last_offset: Some(encode_u128(response.last_offset.get())),
        placement: Some(v1::Placement {
            shard_id: response.placement.shard_id.get(),
            ring_epoch: response.placement.ring_epoch.get(),
            sequence: Some(encode_u128(response.placement.sequence.get())),
        }),
    }
}
...
fn encode_fetch_batch(request_id: u128, batch: FetchedBatch) -> v1::FetchBatch {
    v1::FetchBatch {
        ...
        placement: Some(v1::Placement {
            ...
            sequence: Some(encode_u128(batch.placement.sequence.get())),
        }),
    ...
    async fn standard_grpc_clients_append_and_fetch() {
        ...
        let mut fetched = consumer
            .fetch(v1::FetchRequest {
                ...
                topic_id: Some(encode_u128(9)),
                partition_id: 0,
                start_offset: Some(encode_u128(0)),
                max_bytes: 1024,

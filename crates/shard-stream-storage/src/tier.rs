...
impl LocalObjectTier {
    ...
    pub fn offload_pack(
        ...
        publish_manifest(&self.root, self.shard_id, &next)?;
        self.manifest = next;
        Ok(object)
    }
}
...
fn publish_manifest(root: &Path, shard_id: ShardId, manifest: &TierManifest) -> StorageResult<()> {
    manifest.validate(shard_id)?;
...
mod tests {
    ...
    impl TempDir {
        fn new() -> Self {
            ...
            let path = std::env::temp_dir()
                .join(format!("shard-stream-tier-{}-{nonce}", std::process::id()));
    ...
    fn publishes_immutable_objects_and_recovers_from_current_manifest() {
        ...
        let object = tier.offload_pack(7, &source).expect("offload");
    ...
    fn rejects_object_corruption_on_recovery() {
        ...
        let object = tier.offload_pack(0, &source).expect("offload");

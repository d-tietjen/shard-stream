...
const JOURNAL_MAGIC: &[u8; 4] = b"SSJ3";
...
pub struct AtomicGroupSequence {
...
pub enum JournalEvent {
    /// Legacy partition configuration record. New partitions use
    /// `CreatePartition`, which atomically persists their durability policy.
    Configure {
...
fn encode_event(event: &JournalEvent) -> StorageResult<Vec<u8>> {
    let (kind, body) = match event {
        ...
        JournalEvent::CreatePartition {
            ...
            let shard_count = u32::try_from(shards.len())
                .map_err(|_| StorageError::LimitExceeded("too many shards".into()))?;
            let mut body = Vec::with_capacity(16 + 4 + 8 + 4 + shards.len() * 4 + 8 + 4 + 4);
            push_topic_partition(&mut body, *topic_partition);
            push_u64(&mut body, ring_epoch.get());
            push_u32(&mut body, shard_count);
            for shard in shards {
            ...
            push_u64(&mut body, *policy_generation);
            push_u32(&mut body, *replication_factor);
            push_u32(&mut body, *min_in_sync_replicas);
            (CREATE_PARTITION_KIND, body)
        }
        JournalEvent::UpdateReplicationPolicy {
            ...
            if *expected_policy_generation == 0 || *policy_generation <= *expected_policy_generation
            {
                return Err(StorageError::InvalidInput(
                    "replication policy generation must advance".into(),
                ));
            }
            let mut body = Vec::with_capacity(16 + 4 + 8 + 8 + 4 + 4);
            push_topic_partition(&mut body, *topic_partition);
            push_u64(&mut body, *expected_policy_generation);
            push_u64(&mut body, *policy_generation);
            push_u32(&mut body, *replication_factor);
            push_u32(&mut body, *min_in_sync_replicas);
            (UPDATE_REPLICATION_POLICY_KIND, body)
        }
        ...
        JournalEvent::GroupCommit {
            ...
            GROUP_COMMIT_KIND,
            encode_group_marker(*topic_partition, *transaction_id),
        ),
...
fn decode_event(path: &Path, offset: u64, kind: u8, body: &[u8]) -> StorageResult<JournalEvent> {
    ...
    let event = match kind {
        CONFIGURE_KIND => {
            ...
            let shards = decode_shards(path, offset, &mut decoder)?;
            JournalEvent::Configure {
                topic_partition,
                ring_epoch,
                shards,
        ...
        CREATE_PARTITION_KIND => {
            ...
            let replication_factor = decoder.u32()?;
            let min_in_sync_replicas = decoder.u32()?;
            validate_decoded_replication_policy(
                path,
                offset,
                policy_generation,
                replication_factor,
        ...
        UPDATE_REPLICATION_POLICY_KIND => {
            let topic_partition = decode_topic_partition(&mut decoder)?;
            let expected_policy_generation = decoder.u64()?;
            let policy_generation = decoder.u64()?;
            let replication_factor = decoder.u32()?;
            let min_in_sync_replicas = decoder.u32()?;
            validate_decoded_replication_policy(
            ...
            if expected_policy_generation == 0 || policy_generation <= expected_policy_generation {
                return Err(StorageError::corrupt(
                    path,
                    offset,
                    "replication policy generation does not advance",
                ));
            }
            JournalEvent::UpdateReplicationPolicy {
                topic_partition,
        ...
        GROUP_ABORT_KIND => JournalEvent::GroupAbort {
            topic_partition: decode_topic_partition(&mut decoder)?,
            transaction_id: decoder.u128()?,
        },
        _ => {
...
fn encode_group_marker(topic_partition: TopicPartition, transaction_id: u128) -> Vec<u8> {
    let mut body = Vec::with_capacity(36);
    push_topic_partition(&mut body, topic_partition);
    push_u128(&mut body, transaction_id);
    body
}
...
fn validate_replication_policy(
    policy_generation: u64,
    replication_factor: u32,
    min_in_sync_replicas: u32,
) -> StorageResult<()> {
    if policy_generation == 0 {
        return Err(StorageError::InvalidInput(
            "policy generation must be nonzero".into(),
        ));
    }
    ...
    {
        return Err(StorageError::InvalidInput(
            "invalid replication policy".into(),
        ));
    }
    Ok(())
}

fn validate_decoded_replication_policy(
    path: &Path,
    offset: u64,
    policy_generation: u64,
    replication_factor: u32,
    ...
    validate_replication_policy(policy_generation, replication_factor, min_in_sync_replicas)
        .map_err(|_| StorageError::corrupt(path, offset, "invalid replication policy"))
}
...
mod tests {
    ...
    fn round_trips_every_event_and_repairs_partial_tail() {
        ...
        let events = vec![
            JournalEvent::CreatePartition {
                topic_partition: TopicPartition::new(TopicId::new(8), LogicalPartitionId::new(1)),

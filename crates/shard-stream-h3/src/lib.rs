...
fn install_crypto_provider() {
    // Some applications compose dependencies that enable both rustls crypto
    // providers. Select ring explicitly before constructing TLS state.
        ...
        Method::GET => {
            ...
            let fetch = FetchRequest {
                ...
                start_offset: LogicalOffset::new(
                    query
                        .get("offset")
                        .map(|value| parse_u64("offset", value))
                        .transpose()?
                        .unwrap_or(0),
                ),
                max_bytes,
...
fn parse_producer(query: &HashMap<String, String>) -> Result<Option<ProducerIdentity>, RouteError> {
    match (
        ...
        (Some(id), Some(epoch), Some(sequence)) => Ok(Some(ProducerIdentity {
            ...
            event_id: query
                .get("event_id")
                .map(|value| parse_u128("event_id", value).map(shard_stream_core::RecordId::new))
                .transpose()?,
        })),
        _ => Err(RouteError::bad_request(
